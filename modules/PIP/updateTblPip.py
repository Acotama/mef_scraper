...
def main(con,years,opc,tipo):
    ...
    def getProjects():
        ...
        if(opc == 1):
            ...
            con.getCur().execute("""
                select grli_pip_total_priori.cod_snip,
                    p.cod_unif,
                    id,grli_pip_total_priori.ger_direc,grli_pip_total_priori.tipo_pry,grli_pip_total_priori.sector ,grli_pip_total_priori.tipo_inversion,grli_pip_total_priori.cierre,tipo_emergencia,grli_pip_total_priori.exptec_pdf ,grli_pip_total_priori.f_exptec ,grli_pip_total_priori.anio
                    from sp_proyecto(%s) as p
                inner join grli_pip_total_priori on p.cod_unif = grli_pip_total_priori.cod_unif where grli_pip_total_priori.ger_direc is null and p.cod_unif != '2001621'
            """,(str(years),))
            #  nom_proyec is null ORDER BY anio desc
        else:
            con.getCur().execute("""
           SELECT
                    cod_snip,
                    cod_unif,
                    id,ger_direc,tipo_pry,tipo_inversion,cierre,tipo_emergencia,exptec_pdf ,f_exptec ,sector
                FROM grli_pip_total_priori 
                WHERE  (id <> 1000)   ORDER BY anio desc
            """,(str(datetime.now().date()),))
        #  WHERE (id <> 1000) ORDER BY anio desc;
        # and cod_unif ='2466179'
...
# 
    ...
    def updateWithInfoSnip(infoSnipdata, codigoSnip, codigoUnif, idProyecto,data):
        for infoSnip in infoSnipdata:
            ...
            if str(data['sector']).strip() == "" or data['sector'] is None or data['sector'] == " ":
                _sector        = infoSnip['FUNCION']
            else:
                _sector        = data['sector']
            _m_laudo       = infoSnip['MTO_LAUDO'] 
            _m_fianza       = infoSnip['MTO_CartFza']
            _m_concurrente = infoSnip['MTO_CCONCURR_EJE']
            _m_pip         = infoSnip['COSTO_ACTUALIZADO'] + _m_fianza + _m_laudo + _m_concurrente
    ...
    #Datos Finaiceros Primera Opcion
    def update_financiero(infoSiafdata, codigoUnif, idProyecto):
        for infoSiaf in infoSiafdata:
            _m_pim_a_todas_ue    = Decimal(0.00)
            _m_deveng_a_todas_ue = Decimal(0.00)
            _m_deveng_todas_ue = Decimal(0.00)
            _a_financ_a          = Decimal(0.00)
            _pia           = Decimal(0.00)
            _pim           = Decimal(0.00)
            _dev           = Decimal(0.00)
            _comp_anual    = Decimal(0.00)
            _certif        = Decimal(0.00)
            _m_pip         = Decimal(0.00)
            _m_laudo       = Decimal(0.00)
            _m_fianza      = Decimal(0.00)
            _pim_ult = Decimal(0.00)
            _dev_ult = Decimal(0.00)
            _pim_acu = 0.00
            _dev_acu = 0.00
            _dev_acu_todas_ue = 0.00
            _m_viab = 0.00
            _m_exptec = 0.00

            _tipo_proyecto       = infoSiaf['TIPO_FORMATO']
            _m_laudo       = infoSiaf['MTO_LAUDO'] 
            _m_fianza       = infoSiaf['MTO_CartFza']
            _m_concurrente = infoSiaf['MTO_CCONCURR_EJE']
            _m_pip         = infoSiaf['COSTO_ACTUALIZADO'] + _m_fianza + _m_laudo + _m_concurrente
            _m_viab        = infoSiaf['MTO_VIABLE']
            _m_exptec      = infoSiaf['MTO_F15']
            _anio_financ   = '0'
            ### Info Financiera
            ...
            if infoSSI:
                ...
                for e in infoSSI:
                    ...
                    if int(e['SEC_EJEC']) in unidadesEjecutoras:
                        ...
                        _pia           += Decimal(e['MTO_PIA'])
                        _pim           += Decimal(e['MTO_PIM'])
                        _comp_anual    += Decimal(e['MTO_COMPROM'])
                        _certif        += Decimal(e['MTO_CERT'])
                        _anio_financ   = e['NUM_ANIO']
                       
                        _dev           += Decimal(e['MTO_DEVEN']) if _anio_financ != str(years) else 0

                        if _anio_financ == str(years):
                            _pim_ult += Decimal(e['MTO_PIM'])
                            _dev_ult += Decimal(e['MTO_DEVEN'])
                    else:
                        _anio_financ   = e['NUM_ANIO']
                        if _anio_financ == str(years):
                            _m_deveng_todas_ue += twoDec(e['MTO_DEVEN'])


            _ult_anio_ejec_pry_financ = _anio_financ or '0'
            _pim_acu = _pim
            _dev_acu = _dev


            try:
                _a_financ = str(twoDec(Decimal(str(_dev_ult)) / Decimal(str(_pim_ult)) * 100))
            except:
            ...
            print ('|_ Devengado ultimo Año-> ' + str(round(_dev_ult,2)))
            print ('|_ Devengado ultimo Año OTRAS UEI-> ' + str(round(_m_deveng_todas_ue,2)))
            print ('|_ Pim Ultimo Año -> ' + str(round(_pim_ult,2)))

            print ('|_ Devengado acumulado-> ' + str(round(_dev_acu,2)))
            print ('|_ Pim acumulado -> ' + str(round(_pim_acu,2)))
            print ('|_ Monto Laudo -> ' + str(round(_m_laudo,2)))

            print ('|_ Costo Actualizado -> ' + str(round(_m_pip,2)))
            print ('|_ Devengado acumulado Total -> ' + str(round(_m_deveng_a_todas_ue,2)))
            print ('|_ Pim acumulado Total -> ' + str(round(_m_pim_a_todas_ue,2)))
        ...
        for i in projects:
            ...
            if infoSiaf:
                if tipo == "FINANCIERO":
                    ...
                    # update_financiero(infoSiaf, codigoUnif, idProyecto)
